...
def create_and_train_model():
    ...
    return svm_classifier.fit(X_train, np.ravel(y_train))
...
def evaluate_metrics():
    ...
    prec_score = precision_score(y_test, svm_predictions, average='weighted')
    r_score = recall_score(y_test, svm_predictions, average='weighted')
    fscore = f1_score(y_test, svm_predictions, average='weighted')
    print("accuracy score of SVM: %.5f" % acc)
    print("precision score of SVM: %.5f" % prec_score)
    print("recall score of SVM: %.5f" % r_score)
...
def plot_observation():
    ...
    svm_predict = svm_predictions[:200]
    plt.xlabel('X(Time->)')
    plt.ylabel('0 for Benign Traffic(LOW) and 1 for Malicious Traffic(HIGH)')
    plt.plot(y_test_predict, c='g', label="Benign data")
    plt.plot(svm_predict, c='b', label="Malicious data")
    plt.legend(loc='upper left')
    plt.savefig('SVM.png')

    classes = np.unique(y_test)
    fig, ax = plt.subplots(figsize=(5, 3))
    cm = metrics.confusion_matrix(y_test, svm_predictions, labels=classes)
    sns.heatmap(cm, annot=True, fmt='d', cmap=plt.cm.Blues, cbar=False)
    ax.set(xlabel="Pred", ylabel="True", title="Confusion matrix for SVM")
...
if __name__ == '__main__':
    ...
    svm_model = create_and_train_model()

    # predict model
    svm_predictions = predict_model()
